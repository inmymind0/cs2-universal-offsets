...
| `NoSpread1` | `client.dll` | `NoSpread1` | Optional client-spread suppress for the perfect-shot path. Disabled by default — server desync risk. |
| `TraceCreate` | `client.dll` | `TraceCreate` | Pass-through; used as the trace-pipeline entry point for predicted-shot vischecks. |
...
| Function | Module | Signature | Action |
|---|---|---|---|
| `CCSGOInput::CreateMove` | `client.dll` | `CreateMove` | Aimbot's primary tick — runs phase machine, performs angle snap, sets fire latch. |
...
| Function | Module | Signature | Action |
|---|---|---|---|
| `KillFeedbackEmitter` | `client.dll` | `KillFeedbackEmitter` | Detect confirmed kills (fires from engine damage flow, not aimbot lock state). |
| `DamageFeedbackEmitter` | `client.dll` | `DamageFeedbackEmitter` | Selectively skip the HS chirp via GetHitGroup branch (head=1). |
| `EmitSoundByHandle` | `client.dll` | `EmitSoundByHandle` | Universal sound-name capture for filter/log. |
| `StartSoundEvent` | `soundsystem.dll` | `CSosOperatorSystem_StartSoundEvent` | Convergence point that catches by-handle/by-hash variants the by-name path misses (HS dink). |
...
| Function | Module | Signature | Action |
|---|---|---|---|
| `ApplyEconCustomization` | `client.dll` | `ApplyEconCustomization` | Modern paint-apply entry; consumes m_nFallback* fields and queues composite rebuild. |
| `RegenerateWeaponSkin` | `client.dll` | `RegenerateWeaponSkin` | Legacy static-paint pass; called for completeness, NOT what makes modern skins visible. |
| `GetCustomPaintKitIndex` | `client.dll` | `CEconItemView::GetCustomPaintKitIndex` | Read live paint kit to detect rejection. |
...
| `AnimGraphRebuild` | `client.dll` | `AnimGraphRebuild` | Mode=2: destroy CNmGraphInstance and let CAnimGraphControllerManager re-bind from new vdata. |
...
| Function | Module | Signature | Action |
|---|---|---|---|
| `CCSGOInput::CreateMove` | `client.dll` | `CreateMove` | Inject IN_JUMP on landing subtick when bhop enabled and m_iJump latch armed. |
